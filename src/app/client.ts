// src/app/client.ts
import "@/style.css";
import { MAX_GAMEPADS, SHARED_BUFFER_SIZE } from "@/shared/state/sharedInputLayout";
import {
  clearGamepadState,
  createInputContext,
  updateGamepadState,
} from "@/client/input/manager";
import { SHARED_ENGINE_STATE_BUFFER_SIZE } from "@/shared/state/sharedEngineStateLayout";
import {
...
const { RAYCAST_RESULTS_BUFFER_SIZE, COLLISION_EVENTS_BUFFER_SIZE } =
  await import("@/shared/state/sharedPhysicsLayout");
...
const worker = new Worker(new URL("../client/runtime/worker.ts", import.meta.url), {
  type: "module",
});
